...
// import ODESolver to use it for ODE System of our model
...
public class BSimBacterium extends BSimParticle {
	...
	public void setSurfaceAreaGrowthRate(double d) {
		surfaceAreaGrowthRate = d;
	}
	//public void setSurfaceAreaGrowthRate(double d, double b) {
		// overloading this function to calculate the growthrate in each step but just give the same growthrate to the children
	//	surfaceAreaGrowthRate = getLambda();
	//	System.out.printf("lambda in setSurfaceAreaGrowthRate %.10f", getLambda());
	//}

	/*************************************************************************************************/

	//public double calculateGrowthRate(double d, double b) {
		//works : double growthrate in each step
		//double lambda = surfaceAreaGrowthRate*2;
		// put in calculation of lambda here! therefore bind in the ODESolver.. we need a new class that can solve this problem (probably)
	...
	public void grow() {
		//setSurfaceAreaGrowthRate(getLambda()); //with two variables lambda will be calculated with our model
	...
	*/
	...
	public void action() {
		...
		yNew = BSimOdeSolver.rungeKutta23(lamSys, sim.getTime(), y, sim.getDt());
		y = yNew;

		//if(surfaceAreaGrowthRate > 0) {
		//now always growing/replication
		grow();
		System.out.printf("GrowthRate in action after grow() %.4f ", surfaceAreaGrowthRate);
		System.out.print(" ");
		//}
	}
